<template>
  <div class="pt-20 px-5 xl:px-6">
    <div class="w-full mx-auto max-w-3xl pt-48 pb-20">
          ...
          <div class="row-span-2 aspect-video w-full relative">
            ...
            <div class="absolute top-5 right-5">
              <img
                ...
                class="h-6 w-6 cursor-pointer sm:h-10 sm:w-10"
                alt=""
              />
              <img
                v-else
...
const handleSearch = async () => {
  ...
  if (textValue.value.length > 0) {
    ...
    productList.value = combinedResults.map((item) => {
      const isBookmarked = checkBookmarked(item.tool_id);
      console.log(isBookmarked);
      return {
        title: item.title,
    ...
    //remove duplicates in productList array
    const unique = [
      ...new Set(productList.value.map((item) => item.tool_id)),
    ].map((tool_id) => {
      return productList.value.find((item) => item.tool_id === tool_id);
    });

    productList.value = unique;
  }
};
</script>
