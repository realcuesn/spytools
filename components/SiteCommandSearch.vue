...
const handleSearch = async () => {
  ...
  if (textValue.value.length > 0) {
    ...
    const [descriptionResult, anotherColumnResult, tagsResult] =
      await Promise.all([descriptionQuery, anotherColumnQuery, tagsQuery]);
...
<template>
  <div
    class="w-full h-screen fixed top-0 px-2 z-50 bg-black bg-opacity-80 backdrop-blur-lg"
  >
    <div class="w-full relative flex items-center justify-center h-full">
      <XCircleIcon
        class="h-8 w-8 absolute top-4 right-4 cursor-pointer hover:opacity-80 transition duration-300 ease-in-out"
        @click="open = false"
      />
      ...
      <div class="w-full max-w-3xl px-3 mx-auto">
        <form @submit.prevent="handleSearch()" class="w-full flex items-center">
          <input
            ...
            type="text"
            placeholder="Escribe para buscar entre +500 herramientas"
            v-model="textValue"
          />
          <button
            type="submit"
        ...
        <div
          ...
          class="bg-[#272727] py-2 mt-4 px-2 flex flex-col w-full gap-y-3 rounded-md"
        >
          <div
            v-for="result in results"
            @click="handleRedirect(result.tool_id)"
        ...
        <div v-else class="bg-[#272727] py-2 mt-4 px-2 rounded-md text-center">
          <span class="text-[#A3A3A3]">No se han encontrado resultados.</span>
        </div>
      </div>
    </div>
  </div>
